...
public class UnityMassageEvent : MonoBehaviour
{
    /************************************************************************
	 * 유니티메시지 이벤트함수
	 * 
    ...
    private void Start()    // 모든 오브젝트가 준비가 되어있어야 자료를 가져올 수 있기때문에 Awake먼저
    {
    ...
    private void OnDisable()    // 마무리작업으로 용이
    {
        // 스크립트가 비활성화될 때마다 호출
    ...
    private void FixedUpdate()  // 물리적인 충돌에 대해서는 픽스드로
    {
    ...
    private void OnDestroy()    // 플레이어 몬스터등 죽어서 사라질 때 호출된다. 존재자체가 사라질 때
    {
        // 스크립트가 삭제되었을 경우 호출
